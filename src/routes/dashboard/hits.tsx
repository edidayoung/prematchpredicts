...
function HitsPage() {
  const { today, history } = Route.useLoaderData();
  const [currentMonth, setCurrentMonth] = useState(new Date());
  const [selectedPick, setSelectedPick] = useState<Pick | null>(null);

  useAutoRefresh(5);

  // Combine today and history for calendar display
  const allPicks = today ? [today, ...history] : history;

  // Create a map of date -> pick for quick lookups
  const picksByDate = allPicks.reduce((acc, pick) => {
        ...
        <div className="grid grid-cols-7 gap-2">
          {calendarDays.map((day, index) => {
            ...
            return (
              <div
                key={day}
                onClick={() => pick && setSelectedPick(pick)}
                ...
                {pick && (
                  <div className="flex h-full items-center justify-center">
                    <span className={`text-2xl font-bold ${
                      pick.status === "won" ? "text-success" :
                      pick.status === "lost" ? "text-destructive" :
                      pick.status === "push" ? "text-muted-foreground" :
                      "text-[#10B981]"
                    }`}>
